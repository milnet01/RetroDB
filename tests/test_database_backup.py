...
class TestBackupDatabase:
    def test_backup_creates_consistent_copy(self, tmp_path):
        # `tmp_path` is the pytest-managed per-test temp dir — auto-cleaned
        # at test exit, no need for manual TemporaryDirectory() context.
    ...
    def test_backup_works_under_wal_mode(self, tmp_path):
    ...
    def test_backup_with_concurrent_open_writer(self, tmp_path):
    ...
    def test_backup_corrupt_destination_is_removed(self, tmp_path, monkeypatch):
    ...
    def test_backup_missing_destination_dir(self, tmp_path):
        ...
        src = str(tmp_path / 'src.db')
        dst = str(tmp_path / 'subdir' / 'dst.db')  # subdir never created
        _populate(src)
