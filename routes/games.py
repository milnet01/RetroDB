...
def get_trophy_info_for_game(game_title, system_name):
    ...
    try:
        ...
        # Title aliases (game title -> trophy title)
...
# Import scraper functions with fallback
try:
...
_FILTER_CACHE_MAX = 50  # max entries before eviction
...
def _get_filter_options(system_id=None):
    ...
    Args:
        system_id: Optional system ID to scope filter values to a single system.
    """
    ...
    # Single query fetches all comma-separated fields at once
    rows = query(f"""
    ...
    # Build value counts for all fields in one pass over the result set
    ...
    with _filter_cache_lock:
        # Evict oldest BEFORE inserting to stay within bounds
...
def _build_games_query(params, count_only=False, ids_only=False):
    ...
    if params.get('rating'):
        # rating can be a value from any rating system
    ...
    # Exclude (NOT) filters — games with NULL in the field are NOT excluded
...
def api_hltb_save(game_id):
    ...
    try:
        ...
        if confidence is not None:
            ...
            # Normalize to 0-1 range (HLTB lookup returns 0-100)
...
def api_hltb_search():
    ...
    try:
        ...
        # Use provided system_folder, fall back to 'ps4' for PSN trophy pages
