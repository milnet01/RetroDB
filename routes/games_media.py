...
def api_rename_rom(game_id):
    ...
    if not os.path.exists(old_path):
        return error(f'ROM file not found on disk: {old_path}', 404)
...
def api_delete_screenshot(game_id):
    ...
    if not screenshot_to_delete:
        return error('No screenshot specified', code=200)

    if not safe_filename(screenshot_to_delete):
        return error('Invalid screenshot filename', 400)
    ...
    if screenshot_to_delete not in screenshots:
        return error('Screenshot not found', code=200)
